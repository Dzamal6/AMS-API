...
Revision ID: 1b88af689c63
Revises: fbf918ac5c90
Create Date: 2024-07-16 00:32:11.440262
...
revision: str = '1b88af689c63'
down_revision: Union[str, None] = 'fbf918ac5c90'
...
def upgrade():
    # User table
    op.alter_column('users', sa.Column('created', sa.DateTime(), nullable=True, server_default=sa.text('now()')))
    op.alter_column('users', sa.Column('last_modified', sa.DateTime(), nullable=True, server_default=sa.text('now()'), onupdate=sa.text('now()')))
    event.listen(User, 'before_insert', set_created)
    event.listen(User, 'before_update', set_last_modified)

    # Role table
    op.alter_column('roles', sa.Column('created', sa.DateTime(), nullable=True, server_default=sa.text('now()')))
    op.alter_column('roles', sa.Column('last_modified', sa.DateTime(), nullable=True, server_default=sa.text('now()'), onupdate=sa.text('now()')))
    event.listen(Role, 'before_insert', set_created)
    event.listen(Role, 'before_update', set_last_modified)

    # Module table
    op.alter_column('modules', sa.Column('created', sa.DateTime(), nullable=True, server_default=sa.text('now()')))
    op.alter_column('modules', sa.Column('last_modified', sa.DateTime(), nullable=True, server_default=sa.text('now()'), onupdate=sa.text('now()')))
    event.listen(Module, 'before_insert', set_created)
    event.listen(Module, 'before_update', set_last_modified)

    # TokenUsage table
    op.alter_column('token_usage', sa.Column('created', sa.DateTime(), nullable=True, server_default=sa.text('now()')))
    op.alter_column('token_usage', sa.Column('last_modified', sa.DateTime(), nullable=True, server_default=sa.text('now()'), onupdate=sa.text('now()')))
    event.listen(TokenUsage, 'before_insert', set_created)
    event.listen(TokenUsage, 'before_update', set_last_modified)

    # Transcript table
    op.alter_column('transcripts', sa.Column('created', sa.DateTime(), nullable=True, server_default=sa.text('now()')))
    op.alter_column('transcripts', sa.Column('last_modified', sa.DateTime(), nullable=True, server_default=sa.text('now()'), onupdate=sa.text('now()')))
    event.listen(Transcript, 'before_insert', set_created)
    event.listen(Transcript, 'before_update', set_last_modified)

    # ChatSession table
    op.alter_column('chat_sessions', sa.Column('created', sa.DateTime(), nullable=True, server_default=sa.text('now()')))
    op.alter_column('chat_sessions', sa.Column('last_modified', sa.DateTime(), nullable=True, server_default=sa.text('now()'), onupdate=sa.text('now()')))
    event.listen(ChatSession, 'before_insert', set_created)
    event.listen(ChatSession, 'before_update', set_last_modified)

    # Document table
    op.alter_column('documents', sa.Column('created', sa.DateTime(), nullable=True, server_default=sa.text('now()')))
    op.alter_column('documents', sa.Column('last_modified', sa.DateTime(), nullable=True, server_default=sa.text('now()'), onupdate=sa.text('now()')))
    event.listen(Document, 'before_insert', set_created)
    event.listen(Document, 'before_update', set_last_modified)

    # Agent table
    op.alter_column('agents', sa.Column('created', sa.DateTime(), nullable=True, server_default=sa.text('now()')))
    op.alter_column('agents', sa.Column('last_modified', sa.DateTime(), nullable=True, server_default=sa.text('now()'), onupdate=sa.text('now()')))
